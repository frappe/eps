frappe.provide("eps");
frappe.provide("eps.review");

eps.review = class Review {
	constructor({ parent, frm }) {
		...
		this.get_point_logs(this.frm.doctype, this.frm.docname);
	}
	update_points() {
		return frappe
			.xcall("eps.eps.doctype.energy_point_log.energy_point_log.get_energy_points", {
	...
	get_point_logs(doctype, docname) {
		const me = this;
		frappe.call({
			method: "eps.eps.api.get_point_logs",
			args: {
				doctype: doctype,
				docname: docname,
			},
			async: true,
			callback: function (r) {
				me.energy_point_logs = r.message;
				me.update_reviewers();
			},
		});
	}
	...
	show_review_dialog() {
		...
		const review_dialog = new frappe.ui.Dialog({
			...
			primary_action: (values) => {
				...
				frappe
					.xcall("eps.eps.doctype.energy_point_log.energy_point_log.review", {
						doc: {
	...
	update_reviewers() {
		const review_logs = this.energy_point_logs.filter((log) =>
...
function make_review(frm) {
	const review_wrapper = $(frappe.render_template("review"));
	review_wrapper.insertBefore($(".form-tags"));

	if (frappe.boot.energy_points_enabled) {
		if (!frm.is_new()) {
			frm.reviews = new eps.review({
