frappe.provide("eps.energy_points");
...
class UserProfile {
	...
	update_heatmap_data(date_from) {
		frappe
			.xcall("eps.eps.page.user_profile.user_profile.get_energy_points_heatmap_data", {
				user: this.user_id,
	...
	edit_profile() {
		let edit_profile_dialog = new frappe.ui.Dialog({
			...
			primary_action: (values) => {
				...
				frappe
					.xcall("eps.eps.page.user_profile.user_profile.update_profile_info", {
						profile_info: values,
					})
					.then((user) => {
	...
	get_user_rank() {
		return frappe
			.xcall("eps.eps.page.user_profile.user_profile.get_user_rank", {
				user: this.user_id,
			})
			.then((r) => {
	...
	get_user_points() {
		return frappe
			.xcall(
				"eps.eps.doctype.energy_point_log.energy_point_log.get_user_energy_and_review_points",
				{
					user: this.user_id,
				}
			)
...
class UserProfileTimeline extends frappe.ui.BaseTimeline {
	...
	get_user_activity_data() {
		return frappe.xcall("eps.eps.page.user_profile.user_profile.get_energy_points_list", {
	...
	get_activity_timeline_item(data) {
		...
		return {
			...
			is_card: true,
			content: eps.energy_points.format_history_log(data),
		};
	}
...
frappe.provide("eps.ui");
eps.ui.UserProfile = UserProfile;
